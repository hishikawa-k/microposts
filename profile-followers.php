<?php
    ...
    if (isset($_GET['user_id'],$_SESSION['login_user_id'])) {
        ...
        $sql2 = 'SELECT * FROM microposts.follow WHERE follow_id = :follow_id';
        ...
        $statement1->bindParam(':user_id',  $_GET['user_id']);
        $statement2->bindParam(':follow_id',  $_GET['user_id']);
        ...
        $records_follower = $statement2->fetchAll();
        $records_followflag = $followflagstatement->fetchAll();
        if (!empty($records_followflag)) {
             $followflag = TRUE;  
        }
        //配列、$records_follower,$records_followerをループ処理でカンマ区切りのデータにするーーーーーーーーーーーーーーーー5/22ここから
        //SELECT * FROM microposts.user WHERE user_id IN (test,test,test,aaa,aaa)のような形になるため
        
        $followers_array = array();
        //var_dump($records_follower);
        foreach ($records_follower as $value) {
            array_push( $followers_array, $value['user_id'] );
        }
        
        //ex[29,30] $followers_array;
        
        //$follows_data = "IN('".implode("','", $followers_array)."')";
        ...
        $insert = substr(str_repeat(',?', count($followers_array)), 1);
        $sql3 = sprintf('SELECT * FROM microposts.user WHERE id in(%s)', $insert);
        $statement3 = $database->prepare($sql3);


        $statement3->execute($followers_array);

        $records_follower_info = $statement3->fetchAll();
...
?>
                ...
                <div class="col-xs-8">
                    <ul class="nav nav-tabs nav-justified">
                        <li role="presentation" class=""><a href="./profile-posts.php?user_id=<?php echo $_GET['user_id']; ?>">Microposts <span class="badge"></span></a></li>
                        ...
                        <li role="presentation" class="active"><a href="./profile-followers.php?user_id=<?php echo $_GET['user_id']; ?>">Followers <span class="badge"><?php echo count($records_follower_info); ?></span></a></li>
                    </ul>
                    
                    <!--自分のツイート一覧のテンプレート開始-->
                    <ul class="media-list" id="myposts">
<?php
    foreach ($records_follower_info as $value) {
?>
                        <li class="media">
